...
function cachePath(query: string): string {
  // v2: szigorúbb credit-egyezés — régi loose cache érvénytelen
  const hash = createHash("sha256").update(`v2:${query.toLowerCase()}`).digest("hex").slice(0, 16);
...
function splitEjiCredits(mainArtist: string): string[] {
  return mainArtist
...
/**
 * EJI előadó egyezés — credit-szintű, nem token-szétszórás.
 * "bikini" ≠ "Bikini Bandits"; collab OK ha van külön "Bikini" credit.
 */
export function ejiArtistMatchesQuery(mainArtist: string, query: string): boolean {
  const queryTokens = artistTokens(query);
  if (queryTokens.length === 0) return true;
  const qNorm = normalizeArtisjusText(query);
  const credits = splitEjiCredits(mainArtist);
  if (credits.length === 0) return false;

  for (const credit of credits) {
    const cNorm = normalizeArtisjusText(credit);
    if (cNorm === qNorm) return true;
  }

  // Egy szavas query: a credit is egy szavas és ugyanaz (BIKINI ✓, Bikini Bandits ✗)
  if (queryTokens.length === 1) {
    const q = queryTokens[0];
    return credits.some((credit) => {
      const ct = artistTokens(credit);
      return ct.length === 1 && ct[0] === q;
    });
  }

  // Több szavas (pl. beton hofi): valamelyik credit tokenjei ≈ query
  return credits.some((credit) => {
    const ct = artistTokens(credit);
    if (ct.join(" ") === queryTokens.join(" ")) return true;
    const allIn = queryTokens.every((t) => ct.includes(t));
    return allIn && ct.length <= queryTokens.length + 1;
  });
}
...
export async function searchEjiByArtist(
  ...
  const [trackHtml, artistHtml] = await Promise.all([
    ejiFormPost(EJI_TRACK_SEARCH_URL, {
      ...
      // AND / csak mainArtist — ne címszövegből húzzon idegen találatokat
      "appbundle_recordSearch[or]": "0",
